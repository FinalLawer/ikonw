# MyMod —— MC 1.21.1 / NeoForge 21.1.248 模组项目
...
| 命令 | 作用 |
|---|---|
| `gradlew build` | 构建模组，产物在 `build/libs/examplemod-1.0.0.jar` |
...
1. 主类 `src/main/java/com/example/examplemod/ExampleMod.java` 中的 `MODID` 常量
2. 包名（`com.example.examplemod` → 你的 `mod_group_id`）
3. 语言文件目录名（`assets/examplemod` → `assets/<新modid>`）
